...
{
	//return ConstructECAL();
	return ConstructHCAL();
}

G4VPhysicalVolume* SDHCALDetectorConstruction::ConstructECAL()
{
	G4int nLayers = 30 ;
	G4int nPadX = 400 ;
	G4int nPadY = 400 ;
	...
	{
	    rpcVec.push_back( SDHCALRPC::buildSilicon(i , nPadX , nPadY , padSize) ) ;
...
G4VPhysicalVolume* SDHCALDetectorConstruction::ConstructHCAL()
{
	G4int nLayers = 48 ;
	G4int nPadX = 400 ;
	G4int nPadY = 400 ;
	...
	for ( unsigned int i = 0 ; i < static_cast<unsigned int>( nLayers ) ; ++i )
	{
		...
		G4VisAttributes * visAtt = new G4VisAttributes();
		//visAtt->SetColour(G4Colour(G4Colour(0.4, 0.8, 1.0, 0.2)));
		visAtt->SetColour(G4Colour(G4Colour(1, 0., .0, 0.2)));
        visAtt->SetForceSolid(true);
        visAtt->SetVisibility(true); 
